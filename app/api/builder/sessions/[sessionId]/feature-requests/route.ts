...
async function postBuilderFeatureRequest(
  request: Request,
  context: { params: Promise<{ sessionId: string }> }
): Promise<Response> {
  const { sessionId } = await context.params;
  return handlers.requestNativeCapability(request, sessionId);
}

export const POST = withTracedApiHandler(
  "POST /api/builder/sessions/:sessionId/feature-requests",
  postBuilderFeatureRequest
);
